...
WebIDL::ExceptionOr<ImportMap> parse_import_map_string(JS::Realm& realm, ByteString const& input, URL::URL base_url)
{
    ...
    if (TRY(parsed_object.has_property("integrity"))) {
        ...
        normalised_integrity = TRY(normalise_module_integrity_map(realm, integrity.as_object(), base_url));
    }
...
WebIDL::ExceptionOr<ModuleIntegrityMap> normalise_module_integrity_map(JS::Realm& realm, JS::Object& original_map, URL::URL base_url)
{
    // 1. Let normalized be an empty ordered map.
