...
bool Request::cross_origin_embedder_policy_allows_credentials() const
{
    ...
    if (m_policy_container.has<HTML::PolicyContainer>() &&
        m_policy_container.get<HTML::PolicyContainer>().embedder_policy.value != HTML::EmbedderPolicyValue::Credentialless)
        return true;
